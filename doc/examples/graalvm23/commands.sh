...
&& native-image -H:Name=application -J-Xmx4000m -Ob -march=compatibility $([[ -f META-INF/native-image/argfile ]] && echo @META-INF/native-image/argfile) -cp .:BOOT-INF/classes:$(ls -d -1 "$PWD/BOOT-INF/lib/"*.* | tr "\n" ":")'
...
docker run --rm --entrypoint /bin/bash ghcr.io/graalvm/native-image-community:17.0.8 -c "mkdir -p /build/native/nativeCompile"